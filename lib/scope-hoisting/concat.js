...
module.exports = (packager, ast) => {
  ...
  try {
    for (
      var _iterator = assets.values()[Symbol.iterator](), _step;
  ...
  function replaceExportNode(module, originalName, path) {
    let _packager$findExportM = packager.findExportModule(
        module.id,
        originalName,
        replacements
      ),
  ...
  function interop(mod, originalName, path, node) {
    ...
    if (mod.cacheData.isCommonJS && originalName === 'default') {
      ...
      if (!path.scope.getBinding(name)) {
        let _path$getStatementPar = path.getStatementParent().insertBefore(
            DEFAULT_INTEROP_TEMPLATE({
              NAME: t.identifier(name),
              MODULE: node
            })
          ),
          _path$getStatementPar2 = (0, _slicedToArray2.default)(
            _path$getStatementPar,
            1
          ),
    ...
    CallExpression(path) {
      ...
      if (callee.name === '$parcel$require') {
            ...
            if (!isUnusedValue(path)) {
              ...
              if (
                ...
                if (binding && !binding.path.getData('hasESModuleFlag')) {
                  ...
                  try {
                    for (
                      var _iterator2 = binding.constantViolations[
                          Symbol.iterator
                        ](),
                        _step2;
        ...
        if (dep.dynamic && packager.bundle.childBundles.has(mod.parentBundle)) {
          ...
          try {
            for (
              var _iterator3 = mod.parentBundle.siblingBundles[
                  Symbol.iterator
                ](),
                _step3;
              ...
              if (
                !child.isEmpty &&
                packager.options.bundleLoaders[child.type]
              ) {
    ...
    VariableDeclarator: {
      exit(path) {
        ...
        if (t.isObjectPattern(id)) {
          ...
          try {
            for (
              var _iterator4 = path.get('id.properties')[Symbol.iterator](),
                _step4;
              ...
              let _packager$findExportM2 = packager.findExportModule(
                  match[1],
                  key.name,
                  replacements
                ),
        ...
        function replace(id, init, path) {
          ...
          try {
            for (
              var _iterator5 = binding.referencePaths[Symbol.iterator](),
                _step5;
    ...
    MemberExpression: {
      exit(path) {
        ...
        if (match) {
          ...
          let _packager$findExportM3 = packager.findExportModule(
              match[1],
              name,
              replacements
            ),
