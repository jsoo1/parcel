...
function bundleReport(mainBundle, detailed = false) {
  ...
  try {
    for (
      var _iterator = bundles[Symbol.iterator](), _step;
      ...
      if (detailed && bundle.assets.size > 1) {
        ...
        try {
          for (
            var _iterator2 = largestAssets[Symbol.iterator](), _step2;
            ...
            rows.push([
              ...
              logger.chalk.dim(
                logger.chalk.green(prettifyTime(asset.buildTime))
              )
            ]);
...
function* iterateBundles(bundle) {
  ...
  try {
    for (
      var _iterator3 = bundle.childBundles[Symbol.iterator](), _step3;
