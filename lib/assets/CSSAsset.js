        ...
        parsed.walk(node => {
          if (node.type === 'string') {
            const _FROM_IMPORT_RE$exec = FROM_IMPORT_RE.exec(decl.value),
              _FROM_IMPORT_RE$exec2 = (0, _slicedToArray2.default)(
                _FROM_IMPORT_RE$exec,
                2
              ),
  ...
  generate() {
    ...
    return (0, _asyncToGenerator2.default)(function*() {
      ...
      if (_this3.cssModules) {
        js +=
          'module.exports = ' +
          JSON.stringify(_this3.cssModules, null, 2) +
          ';';
      }
      ...
      if (_this3.options.sourceMaps) {
        ...
        if (_this3.previousSourceMap) {
          _this3.previousSourceMap.sources = _this3.previousSourceMap.sources.map(
            v =>
              path.join(
                path.dirname(_this3.relativeName),
                _this3.previousSourceMap.sourceRoot || '',
                v
              )
          );

          if (_this3.sourceMap) {
            _this3.sourceMap = yield new SourceMap().extendSourceMap(
              _this3.previousSourceMap,
              _this3.sourceMap
            );
          } else {
            _this3.sourceMap = yield new SourceMap().addMap(
              _this3.previousSourceMap
            );
          }
        } else if (!_this3.sourceMap) {
          _this3.sourceMap = new SourceMap().generateEmptyMap(
            _this3.relativeName,
            css
          );
        }
