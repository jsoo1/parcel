...
module.exports = {
  ...
  AssignmentExpression(path, asset) {
    ...
    if (
      ...
      if (!scope.hasBinding(identifier.name)) {
        ...
        if (path.scope === scope) {
          let _path$insertBefore = path.insertBefore(
              t.variableDeclaration('var', [
                t.variableDeclarator(t.clone(identifier), right)
              ])
            ),
            _path$insertBefore2 = (0, _slicedToArray2.default)(
              _path$insertBefore,
              1
            ),
  ...
  ImportDeclaration(path, asset) {
    ...
    try {
      for (
        var _iterator = path.node.specifiers[Symbol.iterator](), _step;
  ...
  ExportNamedDeclaration(path, asset) {
    ...
    if (source) {
      ...
      try {
        for (
          var _iterator2 = specifiers[Symbol.iterator](), _step2;
      ...
      try {
        for (
          var _iterator3 = specifiers[Symbol.iterator](), _step3;
...
function addImport(asset, path) {
  ...
  if (lastImport) {
    ...
    var _lastImport$insertAft2 = (0, _slicedToArray2.default)(
      _lastImport$insertAft,
      1
    );

    lastImport = _lastImport$insertAft2[0];
  } else {
    var _path$parentPath$unsh = path.parentPath.unshiftContainer('body', [
      requireCall
    ]);

    var _path$parentPath$unsh2 = (0, _slicedToArray2.default)(
      _path$parentPath$unsh,
      1
    );
...
function safeRename(path, asset, from, to) {
  ...
  if (binding && binding.constant) {
    ...
    let _path$insertAfter = path.insertAfter(
        t.variableDeclaration('var', [
