...
class SASSAsset extends Asset {
  ...
  parse(code) {
    ...
    return (0, _asyncToGenerator2.default)(function*() {
      ...
      let opts =
        (yield _this.getConfig(['.sassrc', '.sassrc.js'], {
          packageKey: 'sass'
        })) || {};
      opts.includePaths = (opts.includePaths
        ? opts.includePaths.map(includePath => path.resolve(includePath))
        : []
      ).concat(path.dirname(_this.name));
      opts.data = opts.data ? opts.data + os.EOL + code : code;
      let type = _this.options.rendition
        ? _this.options.rendition.type
        : path
            .extname(_this.name)
            .toLowerCase()
            .replace('.', '');
      ...
      opts.importer.push((url, prev, done) => {
        ...
        resolver
          .resolve(url, prev === 'stdin' ? _this.name : prev)
  ...
  collectDependencies() {
    ...
    try {
      for (
        var _iterator = this.ast.stats.includedFiles[Symbol.iterator](), _step;
