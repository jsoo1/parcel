...
class Bundle {
  ...
  getSiblingBundle(type) {
    ...
    if (!this.siblingBundlesMap.has(type)) {
      let bundle = new Bundle(
        type,
        Path.join(
          Path.dirname(this.name), // keep the original extension for source map files, so we have
  ...
  getHashedBundleName(contentHash) {
    ...
    let isEntry =
      ...
      Array.from(entryAsset.parentDeps).some(dep => dep.entry); // If this is the main entry file, use the output file option as the name if provided.
  ...
  _package(bundler) {
    ...
    return (0, _asyncToGenerator2.default)(function*() {
      ...
      try {
        for (
          var _iterator3 = _this2.assets[Symbol.iterator](), _step3;
  ...
  _addDeps(asset, packager, included) {
    ...
    return (0, _asyncToGenerator2.default)(function*() {
      ...
      try {
        for (
          var _iterator4 = asset.depAssets.values()[Symbol.iterator](), _step4;
  ...
  getHash() {
    ...
    try {
      for (
        var _iterator5 = this.assets[Symbol.iterator](), _step5;
