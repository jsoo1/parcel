...
class Watcher extends EventEmitter {
  constructor(
    ...
    this.on('ready', () => {
      ...
      try {
        for (
          var _iterator = this.readyQueue[Symbol.iterator](), _step;
  ...
  add(paths) {
    ...
    if (Array.isArray(paths)) {
      ...
      try {
        for (
          var _iterator2 = paths[Symbol.iterator](), _step2;
  ...
  getWatched() {
    ...
    try {
      for (
        var _iterator3 = this.watchedPaths[Symbol.iterator](), _step3;
        !(_iteratorNormalCompletion3 = (_step3 = _iterator3.next()).done);
        _iteratorNormalCompletion3 = true
      ) {
        let path = _step3.value;
        let key = this.options.cwd
          ? Path.relative(this.options.cwd, path)
          : path;
        watchList[key || '.'] = [];
      }
    } catch (err) {
  ...
  getWatchedChildren(path) {
    ...
    try {
      for (
        var _iterator4 = this.watchedDirectories.keys()[Symbol.iterator](),
          _step4;
  ...
  watch(path) {
    if (this.shouldWatchDirs) {
      ...
      if (!parent) {
        ...
        try {
          for (
            var _iterator5 = children[Symbol.iterator](), _step5;
  ...
  _unwatch(paths) {
    ...
    if (Array.isArray(paths)) {
      ...
      try {
        for (
          var _iterator6 = paths[Symbol.iterator](), _step6;
